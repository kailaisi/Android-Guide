...
对于一些只有固定的几种值的数据，我们可以使用枚举来进行表示。

```c
enum em{
	red=0,
	green,//这里会自动+1,也就是值我1
	black
};
```
...
4. 链接阶段，将各个目标文件中的个代码进行绝对地址定位，生成可执行文件。还可以用objcopy生成纯二进制码，去掉文件格式信息。（生成exe文件）。动态链接、静态链接

##### 静态链接

静态链接是在形成可执行程序之前进行链接操作。

在实际开发过程中，会有多个源文件，而且原文件之间存在多种依赖关系。一个源文件会调用另一个源文件中的函数。但是每个源文件都是独立编译的，即一个\*.c形成一个\*.o文件，为了满足前面说的依赖关系，会将这些源文件生成的目标文件进行链接，形成一个可执行的程序。
...
另一个，如果文件修改以后，需要全量的进行更新。更新比较困难。

**优势**

由于可执行程序中已经具备了所有可行程序所需要的任何东西，所以执行速度很快。

##### 动态链接

动态链接出现的原因就是为了剞劂静态链接中提到的两个问题：空间浪费和更新困难。

**原理**

动态链接的基本思想是把程序按照模块拆分为各个相对独立的部分，在程序运行时才将他们链接在一起形成一个完整的程序。

在程序运行的过程中，如果发现需要某个类库，程序会去寻找对应的.o文件，去加载。

这种方式，能够解决静态链接的问题。但是由于是在执行过程中去动态的加载程序的。所以存有一定的性能损失。
...
void write(){
	...
	file=fopen("1.txt","a+");
    //进行信息的写入
	fwrite("hello ,this is a file write" ,1,13,file);
    char buf[1024]={0,};
    //读取数据
    rewind(file);//将游标放到文件的开头位置
    fread(buf,1,13,file);
	fclose(file);
}
```
