...
我们先上一个简单的类图![image-20200404103145645](http://cdn.qiniu.kailaisii.com/typora/20200404103157-62771.png)
...
现在我们已经准备好了一个队列了，当所有的事务内部要执行的操作处理完后，会通过**commit()**或者**commitNowAllowingStateLoss()**,**commitNow()**来进行提交。我们这里跟踪**commit()**的执行，剩下的留以后再慢慢分析。
    ...
    for (int opNum = 0; opNum < numOps; opNum++) {
        ...
        switch (op.cmd) {//通过manager来进行fragment的管理工作
            case OP_ADD:
                ...
                mManager.addFragment(f, false);//这里我们对addFragment来进行一个跟踪处理
                break;
            case OP_REMOVE:
